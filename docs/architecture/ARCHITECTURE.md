...
flowchart TB
    ...
    subgraph ING[Ingestion Subsystem — SCRAPING_SYSTEM.md]
        REG[Source Registry]
        PIPE[Ingestion Pipeline<br/>fetch → parse → extract →<br/>normalize → dedupe → validate]
    end
    ...
    subgraph CORE[Core Data]
        ...
        SENS[(Sensitive Data Vault<br/>matching'e kapalı)]
    end
...
| **Ingestion Pipeline** | Crawl planlama, fetch, parse, normalize, dedupe, expiration, provenance; source isolation ([SCRAPING_SYSTEM.md](SCRAPING_SYSTEM.md)) | İlan içeriğinin anlamlandırılması (Understanding'e devreder) |
...
| **CV Parser / Profile Extractor** | CV → Career Profile alanları + confidence; sensitive alanların ayrıştırılıp vault'a yönlendirilmesi ([AI_SYSTEM.md](AI_SYSTEM.md)) | Kullanıcı onayı olmadan alanı "verified" yapmak |
| **Job Posting Store** | Normalize ilanlar, Canonical Posting'ler, duplicate cluster'lar, freshness/provenance | — |
| **Career Profile Store** | Doğrulanmış/doğrulanmamış profil alanları, preferences, completeness | Sensitive attribute saklama (vault'un işi) |
| **Sensitive Data Vault** | Matching'e kapalı PII/sensitive alanların ayrı saklanması ve erişim denetimi | Matching'e veri sağlamak (bilinçli olarak sağlamaz) |
| **Matching Engine** | Faktör bazlı hybrid scoring, hard requirement kontrolü, ranking ([MATCHING_ENGINE.md](MATCHING_ENGINE.md)) | Açıklama metni üretimi (Explanation Generator ile birlikte çalışır) |
| **Explanation Generator** | Faktör sonuçlarından Match Explanation üretimi (karşılanan/eksik, değerlendirme, CV önerileri) | Skoru değiştirmek |
| **Feed Builder** | Kullanıcı başına sıralı feed'in hesaplanması, cache'lenmesi, tazelenmesi | Skor üretimi |
...
| **Application API** | Kullanıcıya dönük bütün işlemler: auth, profil CRUD, feed servis, arama, feedback, data rights | İş mantığı sahipliği (ilgili alt sisteme delege eder) |
| **Notification & Digest Service** | Eşleşme bildirimleri, digest paketleme, frekans/opt-out yönetimi | Eşleşmenin hesaplanması |
...
3. **Extract & Normalize:** Job Requirement Extractor + Taxonomy mapping → normalize
   **Job Posting** (occupation, requirements, location, work type, salary, provenance,
   confidence).
4. **Dedupe & Validate:** Duplicate Detector cluster'lar → Canonical Job Posting; Data
   Quality Validator eşik altı kayıtları Manual Review Queue'ya düşürür.
5. **Store & Index:** Posting store'a yazılır; freshness score atanır; matching için
   indekslenir.
...
3. Feed Builder ilk feed'i hesaplar; kullanıcı feed'i explanation'larla görür.
...
- **Privacy boundary — Vault:** Sensitive Data Vault'tan Matching Subsystem'e veri akışı
  **yoktur**; bu, kod review + otomatik testle korunan bir sınırdır (NFR-403).
...
| FS-10 | Taxonomy hatası (yanlış occupation mapping) yaygın etki | Sistematik alakasız öneri | Mapping confidence; düşük confidence → Manual Review; taxonomy değişiklikleri versiyonlu ve geri alınabilir |
